...
This folder contains Tcl scripts that build Libero SoC v2023.2 MIV_ESS DGC2 design project for the Future Avalanche Board. The script is executed in Libero SoC to generate the sample design. 
> Design is catered for Libero SoC v2023.2. Using older versions of Libero SoC will result in errors.
...
> A DGC2 Libero design directory is required to have been created to access the .elf file. The hardware needs to be programmed with DGC2 bitstream.
    ...
    5. Select the Debugger tab to set up OpenOCD and GDB
        ...
        b. To set up GDB, browse to manually set the GDB path <SoftConsole-install-directory>\riscv-unknownelf-gcc\bin\riscv64-unknown-elf-gdb.exe.
    6. Click Apply
