...
set project_folder_name_DGC2 MIV_DGC2_BD
set project_dir_DGC2 "./$project_folder_name_DGC2"
set Libero_project_name_DGC2 PF_Avalanche_MIV_RV32_DGC2_BaseDesign
...
if {"$config" == "CFG1"} then {
	...
	if {[file exists $project_dir_DGC2] == 1} then {
		project_exists
	} else {
		create_new_project_label
		...
		file copy ./import/components/IMC_DGC2/hex/miv-ess-blinky-tcm.hex $project_dir_DGC2    
		download_required_direct_cores
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		...
		export_prog_job \
			-job_file_name {PF_Avalanche_MIV_RV32_DGC2_BaseDesign} \
			-export_dir {./MIV_DGC2_BD/designer/BaseDesign/export} \
			-bitstream_file_type {TRUSTED_FACILITY} \
			-bitstream_file_components {}
		save_project
	} else {
